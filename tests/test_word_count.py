...
class BackupWordCountTest:
    ...
    def test_refreshes_stale_stored_counts_before_summarizing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            thread_dir = Path(tmp_dir) / "123_456"
            ThreadArchiveStore(thread_dir).upsert_page(
                1,
            ...
            try:
                connection.execute(
                    ...
                    SET
                        word_count_version = 0,
                        word_count_chinese_chars = 0,
                        word_count_chinese_with_punctuation = 0
                    """
                )
                connection.commit()
            finally:
        ...
        assert summary.chinese_chars == 80
        assert summary.chinese_with_punctuation == 120
        assert row == (WORD_COUNT_VERSION, 80, 120)
